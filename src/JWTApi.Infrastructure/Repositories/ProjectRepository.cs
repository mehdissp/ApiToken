...
namespace JWTApi.Infrastructure.Repositories
{
    public class ProjectRepository : IProjectRepository
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var baseQuery = _context.ProjectUsers.Include(s=>s.project)
                .Where(s => (s.UserId.ToString() == GetUserIdManager(userId) ) && s.project.IsDeleted == false);
              //  .Include(s => s.User)
            ...
            var baseQueryOrdered = baseQuery.OrderByDescending(s => s.project.CreatedAt)
                                           .Skip(skip)
                                           .Take(pageSize);
    ...
    ?.Package?.MaxProjects ?? 0 : 0;
            ...
            var items = result.Select((item, index) => new ProjectWithPackageInfoDto
            {
                ...
                Name = item.Project.project.Name,
                CreatedAt = item.Project.project.CreatedAt,
